	...
	*/
	...
	function Plugin (element, options)
	{
		...
		*/
		...
		setInterval(function(){widget.setClocksTime(widget)}, 1000);
		widget.setClocksTime(widget);
		$(widget.element).find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getLocation', widget.options.language));
		...
		if (widget.options.latitude && widget.options.longitude)
		{
			...
			$(widget.element).find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getLocationSuccess', widget.options.language));
		...
		else
		{
			if (navigator.geolocation)
			{
				...
				{
					if (widget._locationLat !== position.coords.latitude.toFixed(3) || widget._locationLon !== position.coords.longitude.toFixed(3))
					{
				...
				}, function(error)
				{
					...
					$(widget.element).find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getLocationError', widget.options.language));
	...
	$.extend(Plugin.prototype,
	{
		...
		init: function ()
		{
			...
			*/
			...
			widget._tempChart = echarts.init(widget.$element.find('#weatherTempChart-' + widget._uId)[0], widget.options.theme);
			widget._tempChart.showLoading();
			
			widget._humidityChart = echarts.init(widget.$element.find('#weatherHumidityChart-' + widget._uId)[0], widget.options.theme);
			widget._humidityChart.showLoading();
			
			widget._pressureChart = echarts.init(widget.$element.find('#weatherPressureChart-' + widget._uId)[0], widget.options.theme);
			widget._pressureChart.showLoading();
			
			widget._windChart = echarts.init(widget.$element.find('#weatherWindChart-' + widget._uId)[0], widget.options.theme);
			widget._windChart.showLoading();
			
			widget._tempWeekChart = echarts.init(widget.$element.find('#weatherWeekTempChart-' + widget._uId)[0], widget.options.theme);
			widget._tempWeekChart.showLoading();
			
			widget._humidityWeekChart = echarts.init(widget.$element.find('#weatherWeekHumidityChart-' + widget._uId)[0], widget.options.theme);
			widget._humidityWeekChart.showLoading();
			
			widget._pressureWeekChart = echarts.init(widget.$element.find('#weatherWeekPressureChart-' + widget._uId)[0], widget.options.theme);
			widget._pressureWeekChart.showLoading();
			
			widget._windWeekChart = echarts.init(widget.$element.find('#weatherWeekWindChart-' + widget._uId)[0], widget.options.theme);
			widget._windWeekChart.showLoading();
			
			widget._earthquakeChart = echarts.init(widget.$element.find(('#weatherQuakeChart-' + widget._uId))[0], widget.options.theme);
			...
			setTimeout( function()
			{
				$(widget.element).find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getDataFeeds', widget.options.language));
				$(widget.element).find('#imagesContainer-' + widget._uId).append(widget.createWeatherSatImages(widget._uId));
				widget.initWeather();
			},500);
		...
		createWeatherMain: function()
		{
			return '<main class="weatherCube-main tab-content p-0 m-0 rounded-0">' +
				'<div id="tabWeatherCurrent-'+this._uId+'" class="weatherCurrentTab tab-pane fade show active" role="tabpanel" aria-labelledby="tabWeatherCurrentToggle-'+this._uId+'">' +
				...
				'<span class="font-weight-bold mr-3">' + getI18n('weatherCube_currentForecastToday', this.options.language) + '</span>' +
				...
				'<span id="weatherCube-currentTemperature-' + this._uId + '" class="digital-numbers pb-2 pb-sm-1 h1">--</span>' +
				...
				'<span class="mr-1 pb-1">' + getI18n('weatherCube_currentForecastMin', this.options.language) + '</span>' +
				'<span id="weatherCube-currentMinTemperature-' + this._uId + '" class="digital-numbers mr-1 pb-1">--</span>' +
				'<i class="wi ' + this.generateUnitIcon(this.options.units) + ' pb-2 mr-2"></i>' +
				'<span class="mr-1 pb-1">' + getI18n('weatherCube_currentForecastMax', this.options.language) + '</span>' +
				'<span id="weatherCube-currentMaxTemperature-' + this._uId + '" class="digital-numbers mr-1 pb-1">--</span>' +
				'<i class="wi ' + this.generateUnitIcon(this.options.units) + ' pb-2 mr-2"></i>' +
				'</div>' +
				'<div class="weatherCubeMinMaxTemperature d-flex justify-content-end align-items-center mt-3 mr-2">' +
				'<a id="weatherCube-uviIndexTable-'+this._uId+'" href="#" class="pt-3"><span class="mr-2 font-weight-bold">' + getI18n('weatherCube_currentForecastUvi', this.options.language) + '</span></a>' +
				'<img id="weatherCube-uviIndexImage-' + this._uId + '" src="" class="mt-3 mr-2" alt="" title="UV-index" />' +
				'</div>' +
				...
				'<span id="weatherCube-currentDescription-' + this._uId + '" class="font-weight-bold">---------</span>' +
				'</div>' +
				'</div>' +
				'<div class="col-md-6 p-0 weatherCubeTimeBackground">' +
				'<div class="weatherCube-desc font-weight-bold mt-auto text-left p-2">' +
				'<div id="weatherCube-spinner-'+this._uId+'" class="spinner-border spinner-border-sm text-danger mb-1" role="status"></div>' +
				...
				getI18n('weatherCube_latitude', this.options.language)+'<span id="weatherCube-latitude-'+this._uId+'" class="digital-numbers ml-2 badge badge-danger">--.---</span>' +
				'<i class="wi wi-degrees wi-fw"></i>' +
				'</samp>' +
				'<samp class="font-weight-bold small p-1">' +
				getI18n('weatherCube_longitude', this.options.language)+'<span id="weatherCube-longitude-'+this._uId+'" class="digital-numbers ml-2 badge badge-danger">--.---</span>' +
				'<i class="wi wi-degrees wi-fw"></i>' +
				'</samp>' +
				'<hr />' +
				'<samp class="font-weight-bold small p-1">' +
				getI18n('weatherCube_city', this.options.language)+'<span id="weatherCube-city-'+this._uId+'" class="ml-2 badge badge-danger">-------------</span>' +
				'</samp>' +
				'<samp class="font-weight-bold p-1 small">' +
				getI18n('weatherCube_state', this.options.language)+'<span id="weatherCube-state-'+this._uId+'" class="ml-2 badge badge-danger">---</span>' +
				'</samp>' +
				'<samp class="font-weight-bold p-1 small">' +
				getI18n('weatherCube_country', this.options.language)+'<span id="weatherCube-country-'+this._uId+'" class="ml-2 badge badge-danger">---</span>' +
				'</samp>' +
				'</div>' +
				'<div class="ml-auto h-100 pb-3 pt-2 pr-3">' +
				'<span class="badge badge-danger">'+getI18n('weatherCube_local', this.options.language)+'</span>' +
				...
				'</div>' +
				'</div>' +
				'<div id="tabWeatherHour-'+this._uId+'" class="weatherHourTab tab-pane fade" role="tabpanel" aria-labelledby="tabWeatherHourToggle-'+this._uId+'">' +
				'<div id="weatherHoursArea-'+this._uId+'" class="d-flex flex-wrap overflow-hidden"></div>' +
				'</div>' +
				'<div id="tabWeatherDay-'+this._uId+'" class="weatherDayTab tab-pane fade" role="tabpanel" aria-labelledby="tabWeatherDayToggle-'+this._uId+'">' +
				'<div id="weatherDaysArea-'+this._uId+'" class="d-flex flex-wrap overflow-hidden"></div>' +
				'</div>' +
				'<div id="tabWeatherUvi-'+this._uId+'" class="weatherUviTab tab-pane fade" role="tabpanel" aria-labelledby="tabWeatherUviToggle-'+this._uId+'"></div>' +
				'<div id="tabWeatherSat-'+this._uId+'" class="weatherSatTab tab-pane fade" role="tabpanel" aria-labelledby="tabWeatherSatToggle-'+this._uId+'">' +
				'<div id="imagesContainer-'+this._uId+'" class="row no-gutters"></div>' +
				'</div>' +
				'<div id="tabWeatherMoon-'+this._uId+'" class="weatherMoonTab tab-pane fade" role="tabpanel" aria-labelledby="tabWeatherMoonToggle-'+this._uId+'">' +
				this.createWeatherMoonData() +
				'</div>' +
				'<div id="tabWeatherQuake-'+this._uId+'" class="weatherQuakeTab tab-pane fade" role="tabpanel" aria-labelledby="tabWeatherQuakeToggle-'+this._uId+'">' +
				'<div class="row no-gutters">' +
				'<div id="weatherQuakeChart-' + this._uId + '" class="weatherCube-quakeChart col-xl-5 col-lg-6 col-md-12 p-2"></div>' +
				'<div id="weatherQuakesArea-' + this._uId + '" class="weatherCube-quakes col-xl-7 col-lg-6 col-md-12 p-1"></div>' +
				'</div>' +
				'</div>' +
				'<div id="tabWeatherChart-'+this._uId+'" class="weatherChartTab tab-pane fade" role="tabpanel" aria-labelledby="tabWeatherChartToggle-'+this._uId+'">' +
				'<div class="row no-gutters">' +
				...
				'</div>' +
				'</div>' +
				'<div id="tabWeatherMap-'+this._uId+'" class="weatherMapTab tab-pane fade" role="tabpanel" aria-labelledby="tabWeatherMapToggle-'+this._uId+'">' +
		...
		createWeatherNavigation: function()
		{
			return '<nav class="weatherCube-navigation">' +
				...
				'<li class="nav-item">' +
				'<a id="tabWeatherCurrentToggle-'+this._uId+'" title="'+getI18n('weatherCube_viewCurrent', this.options.language)+'" href="#tabWeatherCurrent-'+this._uId+'" role="tab" aria-controls="tabWeatherCurrent-'+this._uId+'" aria-selected="true" class="nav-link active">' +
				'<i class="wi wi-day-sunny"></i>' +
				'</a>' +
				'</li>' +
				'<li class="nav-item">' +
				'<a id="tabWeatherHourToggle-'+this._uId+'" title="'+getI18n('weatherCube_viewHours', this.options.language)+'" href="#tabWeatherHour-'+this._uId+'" role="tab" aria-controls="tabWeatherHour-'+this._uId+'" aria-selected="false" class="nav-link">' +
				'<i class="wi wi-time-5"></i>' +
				'</a>' +
				'</li>' +
				'<li class="nav-item">' +
				'<a id="tabWeatherDayToggle-'+this._uId+'" title="'+getI18n('weatherCube_viewDays', this.options.language)+'" href="#tabWeatherDay-'+this._uId+'" role="tab" aria-controls="tabWeatherDay-'+this._uId+'" aria-selected="true" class="nav-link">' +
				'<i class="wi wi-rain-wind"></i>' +
				'</a>' +
				'</li>' +
				'<li class="nav-item d-none d-sm-block">' +
				'<a id="tabWeatherUviToggle-'+this._uId+'" title="'+getI18n('weatherCube_viewUvi', this.options.language)+'" href="#tabWeatherUvi-'+this._uId+'" role="tab" aria-controls="tabWeatherUvi-'+this._uId+'" aria-selected="false" class="nav-link">' +
				'<i class="wi wi-umbrella"></i>' +
				'</a>' +
				'</li>' +
				'<li class="nav-item">' +
				'<a id="tabWeatherSatToggle-'+this._uId+'" title="'+getI18n('weatherCube_viewSat', this.options.language)+'" href="#tabWeatherSat-'+this._uId+'" role="tab" aria-controls="tabWeatherSat-'+this._uId+'" aria-selected="false" class="nav-link">' +
				'<i class="wi wi-lightning"></i>' +
				'</a>' +
				'</li>' +
				'<li class="nav-item">' +
				'<a id="tabWeatherMoonToggle-'+this._uId+'" title="'+getI18n('weatherCube_viewMoon', this.options.language)+'" href="#tabWeatherMoon-'+this._uId+'" role="tab" aria-controls="tabWeatherMoon-'+this._uId+'" aria-selected="false" class="nav-link">' +
				'<i class="wi wi-moon-waxing-crescent-3"></i>' +
				'</a>' +
				'</li>' +
				'<li class="nav-item">' +
				'<a id="tabWeatherQuakeToggle-'+this._uId+'" title="'+getI18n('weatherCube_viewQuakes', this.options.language)+'" href="#tabWeatherQuake-'+this._uId+'" role="tab" aria-controls="tabWeatherQuake-'+this._uId+'" aria-selected="false" class="nav-link">' +
				'<i class="wi wi-earthquake"></i>' +
				'</a>' +
				'</li>' +
				'<li class="nav-item">' +
				'<a id="tabWeatherChartToggle-'+this._uId+'" title="'+getI18n('weatherCube_viewCharts', this.options.language)+'" href="#tabWeatherChart-'+this._uId+'" role="tab" aria-controls="tabWeatherChart-'+this._uId+'" aria-selected="false" class="nav-link">' +
				'<i class="wi wi-snowflake-cold"></i>' +
				'</a>' +
				'</li>' +
				'<li class="nav-item">' +
				'<a id="tabWeatherMapToggle-'+this._uId+'" title="'+getI18n('weatherCube_viewMaps', this.options.language)+'" href="#tabWeatherMap-'+this._uId+'" role="tab" aria-controls="tabWeatherMap-'+this._uId+'" aria-selected="false" class="nav-link">' +
		...
		createWeatherHeader: function()
		{
			return '<header class="weatherCube-header rounded-0 card-header d-flex flex-row align-items-center p-2">' +
				...
				'<img src="data:image/png;base64,'+getImage(false, false)+'" alt="'+getI18n('weatherCube_title', this.options.language)+'" class="mr-2" />' +
				'<span class="card-text">'+getI18n('weatherCube_title', this.options.language)+'</span>' +
				'</div>' +
				'<div id="currentWeatherInterval-'+this._uId+'" class="weatherCube-header-carousel carousel slide font-weight-bold ml-auto mr-2" data-ride="carousel">' +
		...
		createWeatherFooter: function()
		{
			return '<footer class="weatherCube-footer rounded-0 text-left card-footer d-flex align-items-center flex-wrap-reverse p-2">' +
				...
				'</a>.' +
				'</samp>' +
				'<samp id="weatherCube-detectedIP-'+this._uId+'" class="mr-auto font-weight-bold small"></samp>' +
				...
				getI18n('weatherCube_updated', this.options.language) + '&nbsp;<b id="weatherCube-update-'+this._uId+'">00:00&nbsp;-&nbsp;00-00-0000</b></samp>' +
				'<div id="weatherCube-footerLoader-'+this._uId+'" class="spinner-grow spinner-grow-sm" role="status"></div>' +
				'</div>' +
				'</footer>';
		},
		...
		createWeatherMoonData: function()
		{
			return '<div class="row no-gutters">' +
				...
				'<div class="d-flex">' +
				'<img id="weatherCube-seasonsImage-' + this._uId + '" src="" class="img-fluid d-none d-lg-block" alt="" />' +
				'<div class="alert flex-fill align-items-stretch rounded-0 m-0" role="alert">' +
				'<h5 class="alert-heading">' + getI18n('weatherCube_earthSeasons', this.options.language) + '</h5>' +
				'<div id="weatherCube-seasonsTable-' + this._uId + '" class="table-responsive"></div>' +
				'<hr />' +
				'<h5 class="alert-heading">' + getI18n('weatherCube_earthAstrology', this.options.language) + '</h5>' +
				'<div id="weatherCube-astronomyTable-' + this._uId + '" class="table-responsive"></div>' +
				'</div>' +
				'</div>' +
				'</div>' +
				...
				'<h5 class="alert-heading">' + getI18n('weatherCube_moonPhaseToday', this.options.language) + '</h5>' +
				...
				'<table id="moonCalendar-' + this._uId + '" class="weatherCube-moonTable table table-sm table-striped table-borderless w-100 m-0">' +
				'<thead></thead>' +
				'<tbody></tbody>' +
				'</table>' +
				'</div>' +
		...
		{
			return '<div class="modal fade" id="weatherCubeInfo-'+this._uId+'" tabindex="-1" role="dialog" aria-labelledby="weatherCubeInfoModal-'+this._uId+'" aria-hidden="true">' +
				'<div class="modal-dialog" role="document">' +
				'<div class="modal-content rounded-0">' +
				'<header class="modal-header bg-danger rounded-0 p-2">' +
				'<h6 class="modal-title text-warning font-weight-bold" id="weatherCubeInfoModal-'+this._uId+'">' + getI18n('weatherCube_vendors', this.options.language) + '</h6>' +
				'</header>' +
				...
				'<img src="data:image/png;base64,' + getImage('weatherInfo', 1) + '" class="img-fluid weatherInfoImage" alt="Sat24" />' +
				'</a>' +
				'<a href="https://www.esri.com/en-us/home" title="Esri">' +
				'<img src="data:image/png;base64,' + getImage('weatherInfo', 2) + '" class="img-fluid weatherInfoImage" alt="Esri" />' +
				'</a>' +
				'<a href="https://openweathermap.org/" title="OpenWeatherMap">' +
				'<img src="data:image/png;base64,' + getImage('weatherInfo', 3) + '" class="img-fluid weatherInfoImage" alt="OpenWeatherMap" />' +
				'</a><br />' +
				'<a href="https://www.meteoalarm.eu/" title="MeteoAlarm">' +
				'<img src="data:image/png;base64,' + getImage('weatherInfo', 4) + '" class="img-fluid weatherInfoImage" alt="MeteoAlarm" />' +
				'</a>' +
				'<a href="https://www.usgs.gov/" title="USGS">' +
				'<img src="data:image/png;base64,' + getImage('weatherInfo', 5) + '" class="img-fluid weatherInfoImage" alt="USGS" />' +
				'</a>' +
				'<a href="https://www.usno.navy.mil/" title="US Navy">' +
				'<img src="data:image/png;base64,' + getImage('weatherInfo', 6) + '" class="img-fluid weatherInfoImage" alt="US Navy" />' +
				'</a>' +
				'</div>' +
				'</div>' +
		...
		{
			return '<div class="modal fade" id="weatherCubeVersion-'+this._uId+'" tabindex="-1" role="dialog" aria-labelledby="weatherCubeInfoVersion-'+this._uId+'" aria-hidden="true">' +
				'<div class="modal-dialog" role="document">' +
				'<div class="modal-content rounded-0">' +
				'<header class="modal-header bg-danger rounded-0 p-2">' +
				'<h6 class="modal-title text-warning font-weight-bold" id="weatherCubeInfoVersion-'+this._uId+'">' + this.capitalizeFirstLetter(this._name) + '&nbsp;' + this._version + '</h6>' +
				'</header>' +
		...
		{
			return '<div class="weatherCube-modal modal fade" id="weatherCubeAlerts-'+this._uId+'" tabindex="-1" role="dialog" aria-labelledby="weatherCubeAlerts-'+this._uId+'" aria-hidden="true">' +
				...
				'<div id="alertsInterval-' + this._uId + '" class="weatherCube-alertsCarousel carousel slide" data-interval="true" data-ride="carousel" data-interval="4000"> ' +
				...
				'<button type="button" class="btn btn-info btn-sm" data-dismiss="modal">' + getI18n('weatherCube_close', this.options.language) + '</button>' +
				'</footer>' +
				'</div>' +
				'</div>' +
				'</div>' +
		...
		{
			return '<div class="weatherCube-modal modal fade" id="weatherCubeBeaufort-'+this._uId+'" tabindex="-1" role="dialog" aria-labelledby="weatherCubeBeaufort-'+this._uId+'" aria-hidden="true">' +
				'<div class="modal-dialog modal-full" role="document">' +
				'<div class="modal-content rounded-0">' +
				...
				'<span title="' + getI18n('weatherCube_beaufortScale_0', this.options.language) + ': <1 mph (<0.3 m/s)">0</span>' +
				'</th>' +
				'<th style="background-color:#CCFFFF;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_1', this.options.language) + ': 1-3&nbsp;mph (0.3-1.5&nbsp;m/s)">1</span>' +
				'</th>' +
				'<th style="background-color:#99FFCC;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_2', this.options.language) + ': 4-7&nbsp;mph (1.6-3.3&nbsp;m/s)">2</span>' +
				'</th>' +
				'<th style="background-color:#99FF99;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_3', this.options.language) + ': 8-12&nbsp;mph (3.4-5.5&nbsp;m/s)">3</span>' +
				'</th>' +
				'<th style="background-color:#99FF66;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_4', this.options.language) + ': 13-18&nbsp;mph (5.5-7.9&nbsp;m/s)">4</span>' +
				'</th>' +
				'<th style="background-color:#99FF00;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_5', this.options.language) + ': 19-24&nbsp;mph (8.0-10.7&nbsp;m/s)">5</span>' +
				'</th>' +
				'<th style="background-color:#CCFF00;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_6', this.options.language) + ': 25-31&nbsp;mph (10.8-13.8&nbsp;m/s)">6</span>' +
				'</th>' +
				'<th style="background-color:#FFFF00;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_7', this.options.language) + ': 32-38&nbsp;mph (13.9-17.1&nbsp;m/s)">7</span>' +
				'</th>' +
				'<th style="background-color:#FFCC00;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_8', this.options.language) + ': 39-46&nbsp;mph (17.2-20.7&nbsp;m/s)">8</span>' +
				'</th>' +
				'<th style="background-color:#FF9900;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_9', this.options.language) + ': 47-54&nbsp;mph (20.8-24.4&nbsp;m/s)">9</span>' +
				'</th>' +
				'<th style="background-color:#FF6600;">' +
				'<span title="S' + getI18n('weatherCube_beaufortScale_10', this.options.language) + ': 55-63&nbsp;mph (24.5-28.4&nbsp;m/s)">10</span>' +
				'</th>' +
				'<th style="background-color:#FF3300;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_11', this.options.language) + ': 64-72&nbsp;mph (28.5-32.6&nbsp;m/s)">11</span>' +
				'</th>' +
				'<th style="background-color:#FF0000;">' +
				'<span title="' + getI18n('weatherCube_beaufortScale_12', this.options.language) + ': ≥73&nbsp;mph (≥32.7&nbsp;m/s)">12</span>' +
				'</th>' +
				'</tr>' +
				'<tr class="text-center">' +
				...
				'<td colspan="6"><h6>' + getI18n('weatherCube_beaufortForce_1', this.options.language) + '</h6></td>' +
				'<td colspan="2"><h6>' + getI18n('weatherCube_beaufortForce_2', this.options.language) + '</h6></td>' +
				'<td colspan="2"><h6>' + getI18n('weatherCube_beaufortForce_3', this.options.language) + '</h6></td>' +
				'<td colspan="2"><h6>' + getI18n('weatherCube_beaufortForce_4', this.options.language) + '</h6></td>' +
		...
		{
			return '<div class="weatherCube-modal modal fade" id="weatherCubeUviModal-'+this._uId+'" tabindex="-1" role="dialog" aria-labelledby="weatherCubeUviModal-'+this._uId+'" aria-hidden="true">' +
				'<div class="modal-dialog modal-full" role="document">' +
				'<div class="modal-content rounded-0">' +
				'<div class="modal-body">' +
				...
				'<samp class="h4 font-weight-bold">'+getI18n('weatherCube_currentIndexLow', this.options.language)+'</samp>' +
				'</th>' +
				'<th colspan="2" class="bg-success">' +
				'<samp class="h4 font-weight-bold">'+getI18n('weatherCube_currentIndexModerate', this.options.language)+'</samp>' +
				'</th>' +
				'<th colspan="3" class="bg-warning">' +
				'<samp class="h4 font-weight-bold">'+getI18n('weatherCube_currentIndexHigh', this.options.language)+'</samp>' +
				'</th>' +
				'<th colspan="2" class="bg-danger">' +
				'<samp class="h4 font-weight-bold">'+getI18n('weatherCube_currentIndexVeryHigh', this.options.language)+'</samp>' +
				'</th>' +
				'<th colspan="1" class="bg-secondary">' +
				'<samp class="h4 font-weight-bold">'+getI18n('weatherCube_currentIndexExtreme', this.options.language)+'</samp>' +
				'</th>' +
				'</tr>' +
				'</thead>' +
				'<tbody>' +
		...
		{
			return '<div class="weatherCube-modal modal fade" id="weatherCubeTimeModal-'+this._uId+'" tabindex="-1" role="dialog" aria-labelledby="weatherCubeTimeModal-'+this._uId+'" aria-hidden="true">' +
				'<div class="modal-dialog modal-full" role="document">' +
				'<div class="modal-content rounded-0">' +
				'<div class="modal-body">' +
				'<img src="https://api.usno.navy.mil/imagery/earth.png?date=tomorrow&time=10:13" class="img-fluid card-img" alt="Current Time Zone" />' +
				'<footer class="d-flex justify-content-end align-items-center rounded-0 p-2">' +
				'<button type="button" class="btn btn-info btn-sm" data-dismiss="modal">' + getI18n('weatherCube_close', this.options.language) + '</button>' +
				'</footer>' +
				'</div>' +
		...
		createEarthquakePopup: function(strength, tsunami)
		{
			return '<div class="earthquake-popup">' +
				'<h6 class="text-center text-warning">Earthquake</h6>' +
		...
		createWeatherModalCopyright: function()
		{
			...
			return '<footer class="modal-footer d-flex align-items-center bg-dark rounded-0 p-2">' +
				'<span class="text-warning font-weight-bold mr-auto h6 pt-2">Copyright&nbsp;&copy;&nbsp;2007&nbsp;-&nbsp;' + new Date().getFullYear().toString() + ',&nbsp;<a href="https://mcx-systems.net">MCX-Systems&reg;</a>.</span>' +
				'<button type="button" class="btn btn-info btn-sm" data-dismiss="modal">' + getI18n('weatherCube_close', this.options.language) + '</button>' +
				'</footer>';
		},
		...
		bindEvents: function()
		{
			...
			*/
			plugin.$element.on('click touchstart' + '.' + plugin._name, '.weatherCube-alertDialog', function()
			{
				...
				$.each(plugin._dataResult, function(i, item)
				{
					...
					if (i > 0)
					{
						...
						alerts += '<div id="alertsIntervalItem-' + plugin._uId + '-' + i + '" class="table-responsive carousel-item ' + active + '">' +
				...
				plugin.$element.find('.weatherCube-alertsCarousel table').addClass('table table-sm table-striped table-bordered text-white');
				...
				$('.modal-backdrop').attr('id', 'backdrop-' + plugin._uId).appendTo(plugin.$element.find('#weatherCube-' + plugin._uId));
			});
			
			plugin.$element.on('click touchstart' + '.' + plugin._name, '#weatherCubeInfoToggle-' + plugin._uId, function()
			{
			...
			plugin.$element.on('hidden.bs.modal' + '.' + plugin._name, '#weatherCubeBeaufort-' + plugin._uId, function ()
			{
			...
			plugin.$element.on('shown.bs.modal' + '.' + plugin._name, '#weatherCubeBeaufort-' + plugin._uId, function ()
			{
			...
			plugin.$element.on('click touchstart' + '.' + plugin._name, '#weatherTab-' + plugin._uId + ' a', function(e)
			{
				e.preventDefault();
				$(this).tab('show');
			});
			...
			plugin.$element.on('click touchstart' + '.' + plugin._name, '#weatherCube-uviIndexTable-' + plugin._uId, function(e)
			{
				e.preventDefault();
			...
			{
				e.preventDefault();
				$('body').append(plugin.createWeatherTimeModal());
				$('#weatherCubeTimeModal-' + plugin._uId).modal({
					show: true,
					keyboard: false,
					backdrop: true
				});
				...
				// Appending modal background inside the div
				$('.modal-full .modal-content').height($(document).innerHeight());
			});
			...
			plugin.$element.on('click touchstart' + '.' + plugin._name, function()
			{
				...
				if (nextIndex === (numClicksRequired - 1) || oldestIndex > 0)
				{
					...
					{
						plugin.$element.find('#weatherCube-' + plugin._uId).append(plugin.createWeatherVersionModal());
		...
		createWeatherSatImages: function()
		{
			...
			if (widget.options.satImageEnabled)
			{
				...
				if (result.length)
				{
					...
					widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getSatImages', widget.options.language));
					...
					$.each(result, function(i, value)
					{
						...
						satImage += '<div class="col-sm-12 col-md-6 col-xl-3">' +
							'<div id="imageContainerLoader_'+i+'_'+widget._uId+'" class="weatherCube-loader-overlay">'+widget.createWeatherLoader()+'</div>' +
							'<img id="SatImage-' + widget._uId + '-' + i + '" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAoAAAAHgAQMAAAAPH06nAAAAA1BMVEUAAACnej3aAAAAAXRSTlMAQObYZgAAADxJREFUeNrtwQENAAAAwiD7pzbHN2AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEHSX4AABBztEwQAAAABJRU5ErkJggg==" alt="Sat Image for region ' +
						...
						if (value['animated'])
						{
							...
							if ($.inArray(value['type'], satAnimatedImageTypes) < 0)
							{
								if (widget.options.debug)
								{
									console.error('Selected image type is invalid: ' + value['type'] + '. Allowed types are: ' + satAnimatedImageTypes.toString());
								}
						...
						else
						{
							if ($.inArray(value['type'], satVisualImageTypes) < 0)
							{
								if (widget.options.debug)
								{
									console.error('Selected image type is invalid: ' + value['type'] + '. Allowed types are: ' + satVisualImageTypes.toString());
								}
						...
						let opts = {
							...
							success: function(e)
							{
								widget.$element.find('#SatImage-' + widget._uId + '-' + i).attr('src', e.imageElem.src);
								widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getSatSuccess', widget.options.language));
								if (widget.options.debug)
								{
									console.info('Successfully loaded satellite image for region- ' + value['region']);
								}
								widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_ready', widget.options.language));
							},
							error: function()
							{
								widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getSatErrors', widget.options.language));
								if (widget.options.debug)
								{
			...
			setTimeout( function()
			{
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getSatAll', widget.options.language));
				setTimeout( function()
				{
		...
		createWeatherMap: function()
		{
			...
			let standard = leaflet.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
				maxZoom: 19,
				attribution: 'MCX-Systems © <a href="https://www.openstreetmap.org/copyright" target="_blank">OpenStreetMap</a> contributors</a>'
			});
			...
			let esriImagery = leaflet.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}.jpg', {
				maxZoom: 19,
				attribution: 'MCX-Systems © Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community',
			});
			
			let esriStreet = leaflet.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}', {
				maxZoom: 19,
				attribution: 'MCX-Systems © Esri, DeLorme, NAVTEQ, USGS, Intermap, iPC, NRCAN, Esri Japan, METI, Esri China (Hong Kong), Esri (Thailand), TomTom, 2012'
			});
			
			let lat = widget._locationLat;
			let lon = widget._locationLon;
			
			const baseAttr = 'MCX-Systems © Weather from <a href="https://openweathermap.org/" title="World Map and worldwide Weather Forecast online">OpenWeatherMap</a>';
			...
			let overlayMaps = {};
			overlayMaps[getI18n('weatherCube_currentForecastPrecipitation', widget.options.language)] = openWeatherPrecipitation;
			...
			overlayMaps[getI18n('weatherCube_currentForecastPressure', widget.options.language)] = openWeatherPressure;
			overlayMaps[getI18n('weatherCube_currentForecastRain', widget.options.language)] = openWeatherRain;
			overlayMaps[getI18n('weatherCube_currentForecastTemperature', widget.options.language)] = openWeatherTemp;
			overlayMaps[getI18n('weatherCube_currentForecastEarthquakes', widget.options.language)] = widget._earthquakesArray;
			...
			{
				popup.setLatLng(e.latlng).setContent(getI18n('weatherCube_mapClick', widget.options.language) + e.latlng.toString()).openOn(widget._weatherMap);
		...
		getWeatherQuakesData: function()
		{
			...
			if (quakes_status === 1)
			{
				...
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getQuakesCacheFeed', widget.options.language));
			}
			else
			{
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getQuakesLiveFeed', widget.options.language));
				...
				$.ajax({
					...
					success: function (result)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getQuakesSuccessFeed', widget.options.language));
					...
					error: function (xhr, textStatus, thrownError)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getQuakesErrorFeed', widget.options.language));
						
						widget._retryCount ++;
						
						if ( widget._retryCount <= widget.options.retryLimit )
						{
							// Try again
							widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getFeedTryAgain', widget.options.language));
							$.ajax(this);
		...
		parseWeatherQuakesData: function(data, message)
		{
			...
			$.each(data.features, function(i, item)
			{
				...
				if (Math.round(item.properties.mag * 100) / 100 > 5)
				{
					...
					leaflet.marker([coordLat, coordLon], { icon: greenIcon }).bindPopup(widget.createEarthquakePopup(Math.round(item.properties.mag * 100) / 100, getI18n('weatherCube_earthquakesTsunami', widget.options.language))).addTo(widget._earthquakesArray);
				}
				
				html += '<div class="alert ' + style + ' text-left" role="alert">' +
					'<samp class="mr-1">' + getI18n('weatherCube_earthquakesStatus', widget.options.language) + '<span class="text-uppercase badge bg-primary">' + item.properties.status + '</span></samp>' +
					...
					'<samp class="digital-numbers pb-1">' + Math.round(item.properties.mag * 100) / 100 + '</samp>' +
					'</h1>' +
					...
					'<div class="d-flex justify-content-between">' +
					'<div class="text-left">' +
					'<span class="text-semibold small">' + getI18n('weatherCube_earthquakesLatitude', widget.options.language) + '</span>' +
					'<samp class="text-semibold digital-numbers small ml-2">' + Math.round(item.geometry.coordinates[1] * 100) / 100 + '</samp>' +
					'<i class="wi wi-degrees"></i>' +
					'<hr class="p-0 m-0 mt-1"/>' +
					'<span class="text-semibold small">' + getI18n('weatherCube_earthquakesLongitude', widget.options.language) + '</span>' +
					'<samp class="text-semibold digital-numbers small ml-2">' + Math.round(item.geometry.coordinates[0] * 100) / 100 + '</samp>' +
					'<i class="wi wi-degrees"></i>' +
					'</div>' +
					'<div class="text-center">' +
					'<span class="text-semibold small">' + getI18n('weatherCube_earthquakesTime', widget.options.language) + '</span>' +
					'<samp class="text-semibold small digital-numbers">' + widget.formatTime(item.properties.time, 2, false) + '</samp>' +
					'<hr class="p-0 m-0 mt-1"/>' +
					'<samp class="text-semibold small digital-numbers">' + widget.formatTime(item.properties.time, 1, false) + '</samp>' +
					'</div>' +
					'<div class="text-right">' +
					'<span class="text-semibold small">' + getI18n('weatherCube_earthquakesTsunami', widget.options.language) + '</span>' +
					'<hr class="p-0 m-0 mt-1"/>' +
				...
				series: [
					{
						...
						markPoint: {
							data: [
								{ type: 'min', name: getI18n('weatherCube_earthquakesMaxStrength', widget.options.language) },
								{ type: 'max', name: getI18n('weatherCube_earthquakesMinStrength', widget.options.language) }
							]
						},
						markLine: {
							silent: true,
		...
		getWeatherAstronomyData: function()
		{
			...
			let dayNames = [
				getI18n('weatherCube_monday', widget.options.language), getI18n('weatherCube_tuesday', widget.options.language), getI18n('weatherCube_wednesday', widget.options.language),
				getI18n('weatherCube_thursday', widget.options.language), getI18n('weatherCube_friday', widget.options.language), getI18n('weatherCube_saturday', widget.options.language),
				getI18n('weatherCube_sunday', widget.options.language)
			];
			
			let monthNames = [
				getI18n('weatherCube_january', widget.options.language), getI18n('weatherCube_february', widget.options.language), getI18n('weatherCube_march', widget.options.language), getI18n('weatherCube_april', widget.options.language),
				getI18n('weatherCube_may', widget.options.language), getI18n('weatherCube_june', widget.options.language), getI18n('weatherCube_july', widget.options.language), getI18n('weatherCube_august', widget.options.language),
				getI18n('weatherCube_september', widget.options.language), getI18n('weatherCube_october', widget.options.language),
				getI18n('weatherCube_november', widget.options.language), getI18n('weatherCube_december', widget.options.language)
			];
			
			let phaseNames = [
				getI18n('weatherCube_moonNewMoon', widget.options.language), getI18n('weatherCube_moonNewMoon', widget.options.language),
				getI18n('weatherCube_moonWaxingCrescent', widget.options.language), getI18n('weatherCube_moonWaxingCrescent', widget.options.language), getI18n('weatherCube_moonWaxingCrescent', widget.options.language),
				getI18n('weatherCube_moonWaxingCrescent', widget.options.language),
				getI18n('weatherCube_moonFirstQuarter', widget.options.language), getI18n('weatherCube_moonFirstQuarter', widget.options.language), getI18n('weatherCube_moonFirstQuarter', widget.options.language),
				getI18n('weatherCube_moonWaxingGibbous', widget.options.language), getI18n('weatherCube_moonWaxingGibbous', widget.options.language), getI18n('weatherCube_moonWaxingGibbous', widget.options.language),
				getI18n('weatherCube_moonWaxingGibbous', widget.options.language), getI18n('weatherCube_moonWaxingGibbous', widget.options.language),
				getI18n('weatherCube_moonFullMoon', widget.options.language), getI18n('weatherCube_moonFullMoon', widget.options.language),
				getI18n('weatherCube_moonWaningGibbous', widget.options.language), getI18n('weatherCube_moonWaningGibbous', widget.options.language), getI18n('weatherCube_moonWaningGibbous', widget.options.language),
				getI18n('weatherCube_moonWaningGibbous',widget.options.language), getI18n('weatherCube_moonWaningGibbous', widget.options.language),
				getI18n('weatherCube_moonLastQuarter', widget.options.language), getI18n('weatherCube_moonLastQuarter', widget.options.language), getI18n('weatherCube_moonLastQuarter', widget.options.language),
				getI18n('weatherCube_moonWaningCrescent', widget.options.language), getI18n('weatherCube_moonWaningCrescent', widget.options.language), getI18n('weatherCube_moonWaningCrescent', widget.options.language),
			...
			let moonImage = '<img src="data:image/png;base64,' + getImage('moon', phase) + '" class="img-fluid mt-3 mb-4" alt="' + phaseNames[phase] + '" />';
			...
			{
				// Winter background
				widget.$element.find('#weatherCube-seasonsImage-' + widget._uId).attr('src', 'data:image/png;base64,' + getImage('weatherBackground', 0));
			}
			else if (currentMonth >= 3 && currentMonth <= 5)
			{
				// Spring background
				widget.$element.find('#weatherCube-seasonsImage-' + widget._uId).attr('src', 'data:image/png;base64,' + getImage('weatherBackground', 1));
			}
			else if (currentMonth >= 6 && currentMonth <= 8)
			{
				// Summer background
				widget.$element.find('#weatherCube-seasonsImage-' + widget._uId).attr('src', 'data:image/png;base64,' + getImage('weatherBackground', 2));
			}
			else if (currentMonth >= 9 && currentMonth <= 11)
			{
				// Fall background
				widget.$element.find('#weatherCube-seasonsImage-' + widget._uId).attr('src', 'data:image/png;base64,' + getImage('weatherBackground', 3));
			}
			
			let zodiac = widget.getZodiacSign(month, day, zodiacSigns);
			let zod = '<table class="table table-striped table-bordered w-100">' +
				'<tr>' +
				'<td><img src="data:image/png;base64,' + getImage('zodiac', zodiacSigns.indexOf(zodiac)) + '" alt="' + zodiac + '" /></td>' +
				'</tr>' +
				'<tr>' +
			...
			monthSpan.innerHTML = getI18n('weatherCube_moonPhases', widget.options.language) + monthNames[month - 1] + '-' + year;
		...
		getMoonData: function(location)
		{
			...
			{
				// Get Cache
				data = localStorage.getItem('weather_moon');
				
				let myArray = JSON.parse(data);
				widget.parseWeatherMoonData(myArray, 'Moon data from local storage: ');
				// Expired or Empty Cache
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMoonCacheFeed', widget.options.language));
			}
			else
			{
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMoonLiveFeed', widget.options.language));
				
				$.ajax({
					url: apiMoonDataUrl + '?coords=' + encodeURIComponent(location) + '&date=' + (dt.getMonth() + 1) + '/' + dt.getDate() + '/' + dt.getFullYear() + '&tz=' + -dt.getTimezoneOffset() / 60,
					cache: false,
					success: function(result)
					{
						if (result == null)
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMoonSuccessFeed', widget.options.language));
						widget._retryCount = 0;
					...
					error: function(xhr, textStatus, thrownError)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMoonErrorFeed', widget.options.language));
				...
				setTimeout(function()
				{
					...
					{
						// Get Cache
						data = localStorage.getItem('weather_seasons');
						
						let myArray = JSON.parse(data);
						widget.parseWeatherSeasonsData(myArray, 'Seasons from local storage: ');
						// Expired or Empty Cache
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getSeasonsCacheFeed', widget.options.language));
					}
					else
					{
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getSeasonsLiveFeed', widget.options.language));
						
						$.ajax({
							url: apiSeasonsDataUrl + '?year=' + dt.getFullYear() + '&tz=' + - dt.getTimezoneOffset() / 60 + '&dst=true',
							cache: false,
							success: function (result)
							{
								if ( result == null )
								...
								widget.setLocalStorage('weather_seasons', JSON.stringify(result), widget.options.cache);
								data = localStorage.getItem('weather_seasons');
								
								let dataArray = JSON.parse(data);
								widget.parseWeatherSeasonsData(dataArray, 'New Seasons Feed Data: ');
								widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getSeasonsSuccessFeed', widget.options.language));
								widget._retryCount = 0;
							...
							error: function (xhr, textStatus, thrownError)
							{
								...
								widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getSeasonsErrorFeed', widget.options.language));
				...
				setTimeout(function()
				{
					...
					{
						// Get Cache
						data = localStorage.getItem('weather_eclipse');
						
						let myArray = JSON.parse(data);
						$.each(myArray.eclipses_in_year, function(i, item)
						{
							widget.$element.find('#currentEclipsesData-' + widget._uId).append('<span class="font-weight-bold small">' + item.event + '<br /></span>');
						});
						// Expired or Empty Cache
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getEclipseCacheFeed', widget.options.language));
					}
					else
					{
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getEclipseLiveFeed', widget.options.language));
						...
						$.ajax({
							...
							success: function (result)
							{
								...
								widget.setLocalStorage('weather_eclipse', JSON.stringify(result), widget.options.cache);
								data = localStorage.getItem('weather_eclipse');
								...
								widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getEclipseSuccessFeed', widget.options.language));
								widget._retryCount = 0;
							...
							error: function (xhr, textStatus, thrownError)
							{
								...
								widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getEclipseErrorFeed', widget.options.language));
		...
		parseWeatherSeasonsData: function(data, message)
		{
			...
			let html = '<table class="table table-striped table-bordered w-100">' +
				'<thead>' +
				'<tr>' +
				'<th scope="col" class="text-left">' + getI18n('weatherCube_earthSeasonsDate', widget.options.language) + '</th>' +
				'<th scope="col" class="text-left">' + getI18n('weatherCube_earthSeasonsName', widget.options.language) + '</th>' +
				'</tr>' +
				'</thead>' +
				'<tbody>';
			...
			{
				html += '<tr>' +
					'<th scope="row" class="text-left digital-numbers">' + item.day + '-' + item.month + '-' + item.year + '</th>' +
					'<td class="text-left font-weight-bold">' + getI18n('weatherCube_earthSeasons' + widget.capitalizeFirstLetter(item.phenom), widget.options.language) + '</td>' +
					'</tr>';
			});
			
			html += '</tbody></table>';
			widget.$element.find('#weatherCube-seasonsTable-' + widget._uId).empty().append(html);
			widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMoonCreate', widget.options.language));
		},
		...
		getWeatherCurrentData: function()
		{
			...
			{
				// Get Cache
				data = localStorage.getItem('weather_current');
				
				let myArray = JSON.parse(data);
				widget.parseWeatherCurrentData(myArray, 'Current from local storage: ');
				// Expired or Empty Cache
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getCurrentCacheFeed', widget.options.language));
			}
			else
			{
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getCurrentLiveFeed', widget.options.language));
				...
				$.ajax({
					...
					success: function(result)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getCurrentSuccessFeed', widget.options.language));
						widget._retryCount = 0;
					...
					error: function(xhr, textStatus, thrownError)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getCurrentErrorFeed', widget.options.language));
		...
		parseWeatherCurrentData: function(data, message)
		{
			...
			if (data.weather[0].icon)
			{
				...
				{
					iconUrl = '<img class="weatherCube-currentForecastImage pl-3 img-fluid" src="data:image/png;base64,' + imageForecastChooser(data.weather[0].icon, widget.options.icons) + '" alt="' + widget.capitalizeFirstLetter(data.weather[0].description) + '" />';
					if (widget.options.debug)
					{
				...
				{
					iconUrl = '<i class="wi ' + wId + ' wi-fw weatherCube-currentDefault display-2 p-3" title="' + widget.capitalizeFirstLetter(data.weather[0].description) + '"></i>';
					if (widget.options.debug)
					{
			...
			widget.$element.find('#weatherCube-currentMinTemperature-' + widget._uId).text(Math.round(data.main.temp_min));
			widget.$element.find('#weatherCube-currentMaxTemperature-' + widget._uId).text(Math.round(data.main.temp_max));
			widget.$element.find('#weatherCube-currentDescription-' + widget._uId).text(widget.capitalizeFirstLetter(data.weather[0].description));
		    ...
		    let html = '<div class="d-flex flex-column mr-4 pt-2">' +
			    ...
			    '</div>' +
			    '<div class="text-left ml-1 p-1">' +
			    '<a class="beaufortScaleModal" href="#" title="'+getI18n('weatherCube_beaufortScale_' + windBeaufortScale, widget.options.language)+'">' +
			    ...
			    '</div>' +
			    '<img src="data:image/png;base64,'+imageWindChooser(direction)+'" alt="'+direction+'" class="img-fluid mr-3" />' +
			    '</div>';
			...
			widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getCurrentParsedFeed', widget.options.language));
		...
		getWeatherForecastData: function()
		{
			...
			// Has Data
			if (forecast_status === 1)
			{
				// Get Cache
				data = localStorage.getItem('weather_forecast');
				
				let myArray = JSON.parse(data);
				widget.parseWeatherForecastData(myArray, 'Forecast from local storage: ');
				// Expired or Empty Cache
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getForecastCacheFeed', widget.options.language));
			}
			else
			{
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getCForecastLiveFeed', widget.options.language));
				...
				$.ajax({
					...
					success: function(result)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getForecastSuccessFeed', widget.options.language));
						widget._retryCount = 0;
					...
					error: function(xhr, textStatus, thrownError)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getForecastErrorFeed', widget.options.language));
		...
		parseWeatherForecastData: function(data, message)
		{
			...
			$.each(weather, function(i, data)
			{
				...
				if (day.icon )
				{
					if ( widget.options.icons === 'tick' )
					{
						iconUrl = '<img class="img-fluid p-2" src="data:image/png;base64,' + imageForecastChooser(day.icon, widget.options.icons) + '" alt="' + widget.capitalizeFirstLetter(day.desc) + '" />';
						if ( widget.options.debug )
						{
							console.info('Icons are set and are of type .svg named: tick.');
						}
					...
					{
						iconUrl = '<i class="wi ' + wId + ' wi-fw weatherCube-currentDefault display-2 p-3" title="' + widget.capitalizeFirstLetter(day.desc) + '"></i>';
						if ( widget.options.debug )
						{
							console.info('Icons are set and are of type weather-icons atrr: wi wi-');
						}
				...
				html += '<div class="card flex-fill rounded-0 weatherDay">' +
					...
					Math.round(day.high) +'<i class="wi ' + widget.generateUnitIcon(widget.options.units) + '"></i></span></p>' +
					...
					'<span class="h4 pt-2">'+direction+'</span>' +
					'</div>' +
					'<img src="data:image/png;base64,'+imageWindChooser(direction)+'" alt="'+direction+'" class="img-fluid ml-1" />' +
					'</div>' +
					'</div>' +
					'</div>' +
					'<footer class="card-footer p-1">' +
					'<i class="wi ' + wId + ' wi-fw mr-2"></i>' +
					'<small class="small font-weight-bold">' + getI18n('weatherCube_main' + widget.capitalizeFirstLetter(day.main), widget.options.language) + '</small>' +
					'</footer>' +
					'</div>';
			});
		...
		parseWeatherHourForecastData: function(data, weather)
		{
			...
			$.each(data.list, function(i, item)
			{
				...
				if (item.weather[0].icon != null)
				{
					if (widget.options.icons === 'tick')
					{
						iconUrl = '<img class="weatherCube-hourForecastImage img-fluid" src="data:image/png;base64,' + imageForecastChooser(item.weather[0].icon, widget.options.icons) + '" alt="' + widget.capitalizeFirstLetter(item.weather[0].description) + '" />';
						if (widget.options.debug)
						{
							console.info('Icons are set and are of type .svg named: tick.');
						}
					...
					{
						iconUrl = '<i class="wi ' + wId + ' wi-fw weatherCube-currentDefault display-4 p-3" title="' + widget.capitalizeFirstLetter(item.weather[0].description) + '"></i>';
						if (widget.options.debug)
						{
							console.info('Icons are set and are of type weather-icons atrr: wi wi-');
						}
				...
				html += '<div class="card flex-fill rounded-0 weatherHour">' +
					...
					'<samp class="small font-weight-bold ml-2">' + widget.dayToString(new Date(item.dt * 1000).getDay(), widget.options.language) + getI18n('weatherCube_viewAt', widget.options.language) + '</samp>' +
					'<samp class="badge bg-danger small m-0 digital-numbers">' + widget.formatTime(item.dt, 3, true) + '</samp>' +
					'</header>' +
					...
					'<footer class="card-footer p-1">' +
					'<i class="wi ' + wId + ' wi-fw mr-2"></i>' +
					'<small class="small font-weight-bold">' + getI18n('weatherCube_main' + widget.capitalizeFirstLetter(item.weather[0].main), widget.options.language) + '</small>' +
					'</footer>' +
					'</div>';
			});
				...
				series: [
					{
						...
						markPoint: {
							data: [
								{ type: 'max', name: getI18n('weatherCube_chartMaxTemperature', this.options.language) },
								{ type: 'min', name: getI18n('weatherCube_chartMinTemperature', this.options.language) }
							]
						},
						markLine: {
							data: [
								{ type: 'average', name: getI18n('weatherCube_chartMaxAveTemperature', this.options.language) }
							]
						}
					},
					{
						...
						markLine: {
							data: [
								{ type: 'average', name: getI18n('weatherCube_chartAveTemperature', this.options.language) }
							]
						}
					},
					{
				...
				series: [
					{
						...
						markLine: {
							data: [
								{ type: 'average', name: getI18n('weatherCube_chartAveWindSpeed', this.options.language) }
							]
						}
					}
				]
		...
		getWeatherUviData: function()
		{
			...
			{
				// Get Cache
				data = localStorage.getItem('weather_uvi');
				
				let myArray = JSON.parse(data);
				widget.parseWeatherUviData(myArray, 'From Local Storage: ');
				// Expired or Empty Cache
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getUviCacheFeed', widget.options.language));
			}
			else
			{
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getUviLiveFeed', widget.options.language));
				...
				$.ajax({
					...
					success: function(result)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getUviSuccessFeed', widget.options.language));
						widget._retryCount = 0;
					...
					error: function(xhr, textStatus, thrownError)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getUviErrorFeed', widget.options.language));
		...
		parseWeatherUviData: function(data, message)
		{
			...
			$.each(data, function(i, item)
			{
				...
				if (i < 5)
				{
					...
					uviRowIcon += '<td class="p-2"><img src="data:image/png;base64,'+getImage('uvi', Math.floor(uviWeekIndex))+'" class="img-fluid" alt="'+uviWarning+'" /></td>';
					...
					if(i === 0)
					{
						setTimeout(function()
						{
							widget.$element.find('#weatherCube-uviIndexImage-' + widget._uId).attr('src', 'data:image/png;base64,' + getImage('uvi', Math.floor(uviWeekIndex)));
						}, 200);
					}
				}
			});
			...
			let html = '<div class="table-responsive">' +
				...
				'<span class="h5 font-weight-bold">'+getI18n('weatherCube_uviWeek', widget.options.language)+'</span>' +
				'</td>' +
				'</tr>' +
				'</thead>' +
				'<tbody>' +
			...
			widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getUviParsedFeed', widget.options.language));
		},
		...
		getWeatherMeteoAlarmData: function()
		{
			...
			widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMeteoFeed', widget.options.language));
			...
			{
				// Get Cache
				data = localStorage.getItem('meteo_data');
				
				let myArray = JSON.parse(data);
				widget.parseMeteoAlarmData(myArray, 'MeteoAlarm from local storage: ');
				// Expired or Empty Cache
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMeteoCacheFeed', widget.options.language));
			}
			else
			{
				widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMeteoLiveFeed', widget.options.language));
				$.ajax({
					...
					success: function(result)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMeteoSuccessFeed', widget.options.language));
						widget._retryCount = 0;
						return true;
					},
					error: function(xhr, textStatus, thrownError)
					{
						...
						widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMeteoErrorFeed', widget.options.language));
		...
		parseMeteoAlarmData: function(data, message)
		{
			...
			let forToday = '<div id="alertsToday-' + widget._uId + '" class="carousel-item active">' +
				...
				'<span class="mr-2">' + getI18n('weatherCube_' + widget.deCapitalizeFirstLetter(html[0][0]), widget.options.language) + '</span>' +
				'</a>' +
				html[1][0] +
				html[1][1] +
				html[1][2] +
			...
			let forTomorrow = '<div id="#alertsTomorrow-' + widget._uId + '" class="carousel-item">' +
				'<a href="#" class="weatherCube-alertDialog weatherCubeTooltip">' +
				'<span class="mr-2">' + getI18n('weatherCube_' + widget.deCapitalizeFirstLetter(html[0][2]), widget.options.language) + '</span>' +
				'</a>' +
				html[1][4] +
				html[1][5] +
				html[1][6] +
			...
			widget.$element.find('#currentWeatherInterval-' + widget._uId + ' .carousel-inner').empty().append(forToday).append(forTomorrow);
			widget.$element.find('#weatherCube-loaderText-' + widget._uId).text(getI18n('weatherCube_getMeteoParsedFeed', widget.options.language));
		...
		setClocksTime: function(widget)
		{
			...
			const fSeconds = now.toLocaleString('en-US', {timeZone: widget.options.firstTimezone[1], second: 'numeric' });
			const fSecondsDegrees = ((fSeconds / 60) * 360) + 90;
			secondFirstHand.style.transform = `rotate(${fSecondsDegrees}deg)`;
			
			const sSeconds = now.toLocaleString('en-US', {timeZone: widget.options.secondTimezone[1], second: 'numeric' });
			...
			const fMinutes = now.toLocaleString('en-US', {timeZone: widget.options.firstTimezone[1], minute: 'numeric' });
			const fMinutesDegrees = ((fMinutes / 60) * 360) + ((fSeconds / 60) * 6) + 90;
			minutesFirstHand.style.transform = `rotate(${fMinutesDegrees}deg)`;
			
			const sMinutes = now.toLocaleString('en-US', {timeZone: widget.options.secondTimezone[1], minute: 'numeric' });
			...
			const fHour = now.toLocaleString('en-US', {timeZone: widget.options.firstTimezone[1], hour12: false, hour: 'numeric' });
			const fHourDegrees = ((fHour / 12) * 360) + ((fMinutes / 60) * 30) + 90;
			hourFirstHand.style.transform = `rotate(${fHourDegrees}deg)`;
			
			const sHour = now.toLocaleString('en-US', {timeZone: widget.options.secondTimezone[1], hour12: false, hour: 'numeric' });
		...
		getIPLocation: function()
		{
			...
			$.ajax({
				...
				success: function(location)
				{
					...
					$(widget.element).find('#weatherCube-detectedIP-' + widget._uId).text(getI18n('weatherCube_detectedIP', widget.options.language) + ' ' + location.IPv4);
				},
				error: function(xhr, textStatus, thrownError)
				{
					widget._retryCount++;
